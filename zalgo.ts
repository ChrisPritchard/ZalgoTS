...
module ThePony {
    ...
    export interface IZalgoOptions {
        maxUp: number;
        maxMid: number;
        maxDown: number;
        oddsPerChar: number;
    ...
    let defaultOptions: IZalgoOptions = {
        maxUp: 8,
        maxMid: 3,
        maxDown: 8,
        oddsPerChar: 0.5
    ...
    function randFromSet(set: string[]) {
        return set[rand(set.length)];
    }

    function callZalgo(char: string, options: IZalgoOptions) {
    ...
    export function itCOMES(text: string, options?: IZalgoOptions) {
        if (!options)
            options = defaultOptions;
        return text.split("").map(s => callZalgo(s, options)).join("");
    }
}